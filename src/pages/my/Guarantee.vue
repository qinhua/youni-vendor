      ...
      <scroller class="inner-scroller" ref="guaScroller" height="100%" :on-refresh="refresh"
        ...
        <section class="v-items" v-for="item in list" :data-id="item.id">
          <section class="wrap">
            ...
            <div class="info-con">
              <h3>{{item.nickname}}<span :class="['tg',item.statusCls]">{{item.statusTxt}}</span></h3>
              <div class="bottom">
                ...
                <div class="btn-group" v-if="item.status===1">
                  <button type="button" class="btn" @click="pass(item.id)">同意退还
                  </button>
                  <button type="button" class="btn" @click="deny(item.id)">拒绝退还
                  </button>
                </div>
              </div>
            </div>
